...
interface IConferenceInvitationManager {
  ...
  reject(options: { roomId: string }): Promise<void>;
  ...
  implements IConferenceInvitationManager
{
  ...
  private onInvitationAccepted(data: {
    ...
    if (this.service.roomStore.localUser.isInRoom) {
      this.reject({
        roomId,
        reason: TUIInvitationRejectedReason.kInOtherConference,
      });
    }
    this.service.roomStore.addRemoteUser(inviteeInfo as any);
  ...
  private onInvitationRejected(data: {
    ...
    const { status } = data.invitation;
    const inviteeInfo = { userId, userName, avatarUrl, status };
    this.service.roomStore.addInvitee(inviteeInfo as UserInfo);
  ...
  private onInvitationRevokedByAdmin(data: {
    ...
    const { userId } = data.invitation.invitee;
    this.service.roomStore.removeRemoteUser(userId);
